...
import { FiTag, FiCalendar, FiUsers, FiDollarSign, FiClock, FiEye, FiShare2 } from 'react-icons/fi';
...
export default function CouponList() {
  ...
  const handleShareCoupon = async (coupon) => {
    ...
    const shareData = {
      title: `${coupon.code} - Discount Coupon`,
      text: `Get ${coupon.discountType === 'percentage' ? `${coupon.discountValue}% OFF` : `₹${coupon.discountValue} OFF`} ${coupon.minimumOrderAmount > 0 ? `(Min. order: ₹${coupon.minimumOrderAmount})` : ''}\nValid till ${formatDate(coupon.expiryDate)}`,
      url: baseUrl
    };
